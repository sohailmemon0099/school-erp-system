...
import { bonafideCertificateService } from '../services/bonafideCertificateService';
...
const BonafideCertificates = () => {
  ...
  const fetchCertificates = useCallback(async () => {
    try {
      ...
      const response = await bonafideCertificateService.getBonafideCertificates(params);
  ...
  }, [currentPage, searchTerm, statusFilter, purposeFilter]);
  ...
  const fetchStats = useCallback(async () => {
    try {
      const response = await bonafideCertificateService.getCertificateStats();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCertificate) {
        await bonafideCertificateService.updateBonafideCertificate(editingCertificate.id, formData);
        toast.success('Certificate updated successfully');
      } else {
        await bonafideCertificateService.createBonafideCertificate(formData);
  ...
  const handleEdit = (certificate) => {
    ...
    setFormData({
      ...
      purposeDescription: certificate.purposeDescription || '',
  ...
  const handleIssue = async (certificate) => {
    try {
      await bonafideCertificateService.issueBonafideCertificate(certificate.id);
  ...
  const handleDelete = async (certificate) => {
    if (window.confirm('Are you sure you want to delete this certificate?')) {
      try {
        await bonafideCertificateService.deleteBonafideCertificate(certificate.id);
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600">Manage student bonafide certificates</p>
        </div>
        <button
          onClick={() => {
            resetForm();
                  ...
                  <tr key={certificate.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {purposeOptions.find(p => p.value === certificate.purpose)?.label || certificate.purpose}
                      </div>
                      {certificate.purposeDescription && (
